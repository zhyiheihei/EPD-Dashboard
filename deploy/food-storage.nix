# EPD-Dashboard 家庭食品存储看板 · NixOS 部署模块
#
# 在 nixos-config 中接入（ opi5p）：
#   1. flake.nix 增加 input：
#        epd-dashboard.url = "github:<you>/EPD-Dashboard";   # 或本地 path:...
#   2. hosts/opi5p/configuration.nix：
...
#        };
#   3. nixos-secrets 增加 sops 条目（见 docs/deploy.md）。
#
# 服务组成：
#   - epd-food-server.service  常驻 REST API（127.0.0.1，nginx 私网反代）
...
let
  ...
  package =
    if cfg.package != null then cfg.package else pkgs.callPackage ../server/package.nix { };
...
in
{
  options.lantian.food-dashboard = {
    ...
    package = lib.mkOption {
      ...
      default = null;
      description = "覆盖服务端包（默认取本仓库 server/package.nix）";
    };

    tokenFile = lib.mkOption {
      type = lib.types.path;
      description = "API Bearer Token 文件（建议 sops-nix 管理）";
    };
    ...
    port = lib.mkOption {
      type = lib.types.port;
      default = 8386;
      description = "API 监听端口（nixos-config 端口表登记名建议 EpdFoodDashboard）";
    };
